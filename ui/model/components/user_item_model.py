...
class UserItemModel(QWidget):
    ...
    def remove_button_handler(self):
        q = f"delete from {self.item_table} where id = {self.item_id} returning name;"
        res = self.dbHandleClass.execute_single_query(q)
        if res: 
            logger.debug(f"{res} removido")
            self.info_dict = None
            self.updateList.emit(self.item_id)
        
    def edit_user(self):
        update_info = self.register_modal.infoDict.copy()
        q = f"update '{self.item_table}' set name = '{update_info["name"]}', details = '{update_info["details"]}', image_path = '{update_info["image_path"]}' where id = {self.item_id} returning id;"
        res = self.dbHandleClass.execute_single_query(q)
        if res:
            self.info_dict = update_info.copy()
            logger.debug(f"info do id {res} da tabela {self.item_table} foi atualizado")
            self.updateList.emit(self.item_id)
